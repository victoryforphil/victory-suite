...
impl TopicKey{
    ...
    pub fn from_str(display_name: &str) -> TopicKeyHandle{
        let sections = display_name.split("/").map(|s| TopicKeySection::new_generate(s.to_string())).collect();
    ...
    pub fn display_name(&self) -> String{
        self.sections.iter().map(|s| s.display_name.clone()).collect::<Vec<String>>().join("/")
    }

    pub fn to_string(&self) -> String{
        self.sections.iter().map(|s| s.id.clone()).collect::<Vec<String>>().join("/")
    }
