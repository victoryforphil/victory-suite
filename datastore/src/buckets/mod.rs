...
use crate::{datapoints::Datapoint, primitives::{timestamp::{ VicInstantHandle}, Primitives}, topics::TopicKeyHandle};
#[derive(Debug)]
...
mod tests{
    ...
    use crate::{buckets::Bucket, datapoints::Datapoint, primitives::{timestamp::{VicInstant, VicTimecode}, Primitives}, topics::TopicKey};
    ...
    fn test_bucket_read_write_primitives(){
        ...
        bucket.write().unwrap().add_primitive(time.clone(), test_data.clone());

        //Test read using Latest Value
        {
            let bucket_read = bucket.read().unwrap();
            let latest = bucket_read.get_latest_value().unwrap();
            assert_eq!(latest, &test_data, "Latest value is not the same as the test data");
        }
    ...
    fn test_bucket_read_write_datapoints(){
        ...
        //Test read using Latest Value
        {
            ...
            assert_eq!(&data_point.topic, &latest.topic, "Latest value is not the same as the test data");
            assert_eq!(&data_point.time, &latest.time, "Latest value is not the same as the test data");
            assert_eq!(&data_point.value, &latest.value, "Latest value is not the same as the test data");
        ...
        //Test read using all data points
        {
            ...
            assert_eq!(values.len(), 1, "There should be only one value in the bucket");
            assert_eq!(values[0].value, test_data, "The value in the bucket is not the same as the test data");
        }
    }
}